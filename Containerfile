...
ARG CODER_UID=1000
ARG CODER_GID=1000
RUN zypper -n ref && echo -en "\n\n\n" && zypper -n in git-core git-lfs gh glab gitea-tea bat less cnf cnf-bash openssh-clients cnf hostname openQA-client spec-cleaner rpmlint osc obs-service-* \
    ...
    gopls lua-language-server clang-tools ruby taplo java-21-openjdk-headless php8 \
